...
PCA9685Driver::PCA9685Driver(std::uint8_t bus, std::uint8_t address, std::uint16_t min_pulse, std::uint16_t max_pulse)
  : pca9685_(bus, address), min_pulse_(min_pulse), max_pulse_(max_pluse) {}
...
extern "C" ServoArray::Driver* servoarray_driver(const ServoArray::DriverParams& params) {
  return new PCA9685Driver(params.get<std::uint8_t>("bus"), params.get<std::uint8_t>("address"), params.get<unsigned>("min_pulse", 150), params.get<unsigned>("max_pulse", 600));
}
